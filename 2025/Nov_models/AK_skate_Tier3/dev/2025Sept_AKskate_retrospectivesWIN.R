...
# M14_2d1
r4ss::retro(dir = here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', "rsch", 'M14_2d1'),
            years = 0:-ret_yr,
            exe = 'ss')

# M14_2d
r4ss::retro(dir = here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', "rsch", 'M14_2d_fixedcatch'),
            years = 0:-ret_yr,
            exe = 'ss')
...
# load the retrospective models
retro_m14_2d1 <- r4ss::SSgetoutput(dirvec = here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', "rsch", 'M14_2d1', 
                                                   'retrospectives', paste("retro", 0:-ret_yr, sep = "")),
                               verbose = FALSE)
retro_m14_2d <- r4ss::SSgetoutput(dirvec = here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', "rsch", 'M14_2d', 
                                                       'retrospectives', paste("retro", 0:-ret_yr, sep = "")),
                                   verbose = FALSE)
...
retro_res <- list(retrosumm_m14_2d1 = retrosumm_m14_2d1, retrosumm_m14_2d = retrosumm_m14_2d)

# save results
  if (!dir.exists(here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', 'mgmt', "retro"))) {
    dir.create(here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', 'mgmt', "retro"), recursive = TRUE)
  }
  save(retro_res, file = here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', 'mgmt', "retro", "retro_res.RData"))

# Mohns Rho calc ----
rhopath <- here::here(AYR, '2025_Sept_models', 'AK_skate_Tier3', 'mgmt', 'retro')
...
# M14_2d
endyrvec <-retrosumm_m14_2d$endyrs + 0:-10

#SSB
baserun <- retrosumm_m14_2d$SpawnBio %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1) %>% 
  select(M14_2d, Yr)

retrobioLL <- retrosumm_m14_2d$SpawnBioLower %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1,
         run2025 = replist2,
         run2024 = replist3,
  ...
  select(!Label) %>% 
  pivot_longer(!Yr, names_to = 'Peel', values_to = 'Peel_SSBLL')
retrobioUL <- retrosumm_m14_2d$SpawnBioUpper %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1,
         run2025 = replist2,
         run2024 = replist3,
...
retrobio <- retrosumm_m14_2d$SpawnBio %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1,
         run2025 = replist2,
         run2024 = replist3,
...
#recruits
baserun_rec <- retrosumm_m14_2d$recruits %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1) %>% 
  select(M14_2d, Yr)

retrorecLL <- retrosumm_m14_2d$recruitsLower %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1,
         run2025 = replist2,
         run2024 = replist3,
  ...
  select(!Label) %>% 
  pivot_longer(!Yr, names_to = 'Peel', values_to = 'Peel_recLL')
retrorecUL <- retrosumm_m14_2d$recruitsUpper %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1,
         run2025 = replist2,
         run2024 = replist3,
...
retrorec <- retrosumm_m14_2d$recruits %>% 
  filter(Yr <= AYR) %>% 
  rename(M14_2d = replist1,
         run2025 = replist2,
         run2024 = replist3,
...
plot_retro10 <- ((plot_retrossb + plot_retrossb10) /
  ...
  plot_layout(guides = "collect") #& theme(legend.position = 'bottom')
ggsave(path = rhopath,
       "M14_2dCombined_retro_plot.png",plot=plot_retro10,dpi=600,width = 6, height = 8)
