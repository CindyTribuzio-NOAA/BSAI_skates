...
OBS <- read_csv(here::here(dat_path, paste0("confidential_NORPAC_skates_", AYR, ".csv")))
CAS <- read_csv(here::here(dat_path, paste0("confidential_CAS_skates_", AYR, ".csv"))) %>% 
...
#sums up to year only
#A_CATCH<-CATCH[,list(CATCH=sum(CATCH_WEIGHT)),by=c("NAMES","YEAR")]
#A_CATCH<-A_CATCH[order(YEAR,NAMES),]

write.csv(CATCH, here::here(dat_path, paste0("confidential_CAS_SKpart_", AYR, ".csv")))

#compare to previous year----
prevAYR <- read_csv(here::here(dat_path, paste0("confidential_CAS_SKpart", 2023, ".csv"))) %>% 
  select(!'...1') %>% 
  filter(YEAR <= 2022) %>% 
  group_by(YEAR, AGENCY_GEAR_CODE, NAMES) %>% 
  summarize(old_catch = sum(CATCH_WEIGHT))

catch_compare <- CATCH %>% 
  filter(YEAR <= 2022) %>% 
  group_by(YEAR, AGENCY_GEAR_CODE, NAMES) %>% 
  summarize(new_catch = sum(CATCH_WEIGHT)) %>% 
  left_join(prevAYR) %>% 
  mutate(catch_diff = new_catch - old_catch)

ggplot(catch_compare, aes(x = old_catch, y = new_catch))+
  geom_point()+
  geom_abline()
