...
CAS_speccatch <- read_csv(here::here(dat_path, paste0("confidential_CAS_SKpart_", AYR, ".csv"))) %>% 
...
sk_partcatch <- read_csv(here::here(dat_path, paste0("confidential_CAS_SKpart_", AYR, ".csv"))) %>%  
  filter(YEAR == AYR) %>% 
...
write_csv(all_skate_catch, here::here(dat_path, paste0("BSAIsk_totcatch_", AYR, ".csv")))
