...
fitm1 <- nls(m1, 
             ...
             upper = upper)
summary(fitm1)
RichCI <- as.data.frame(confint(fitm1)) %>% 
  rownames_to_column() %>% 
  rename(param = rowname,
         LCI = '2.5%',
         UCI = '97.5%')
Rich_params <- as.data.frame(coef(fitm1)) %>% 
  rownames_to_column() %>% 
  rename(param = rowname,
         est = 'coef(fitm1)') %>% 
  left_join(RichCI)

write_csv(Rich_params, paste0(here::here(getwd(), AYR, 'Nov_models', 'AK_skate_Tier3', 
                                       'mgmt', 'external_Richards_growth', 'AKskate_Richards_params.csv')))
