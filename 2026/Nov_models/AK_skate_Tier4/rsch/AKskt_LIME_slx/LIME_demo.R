...
ggplot(lh2$df %>% dplyr::filter(By=="Age")) + 
  ...
  xlab("Age")
	...
	##----------------------------------------------------------------
catch <- read_csv(here::here(mod_path, "Catch_multifleet.csv"))

data_mymf <- read_csv(here::here(mod_path, "Length_multiyear_multifleet.csv"))
...
## length comps
lcomps <- cldata %>% select(!c(Fleet, Year, Catch))

## observed samples by bin by year
nsamps <- sum(cldata %>% select(!c(Fleet, Year, Catch)))

## multiply observed number of fish in bins by catch, keep fleet information
lweight1 <- ((cldata %>% select(!c(Fleet, Year, Catch))) * cldata$Catch) %>% mutate(Fleet=cldata$Fleet)
...
source(here::here(mod_path, 'Check_Identifiable2.R'))
