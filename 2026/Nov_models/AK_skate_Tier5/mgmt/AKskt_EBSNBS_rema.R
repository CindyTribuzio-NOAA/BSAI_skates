...
# rema plots for EBS, NBS and combined----

EBS_output <- read_csv(paste0(out_path, "/Tier5_AKskt_EBSoutput.csv")) %>% 
  mutate(model_name = "Separate rema")
NBS_output <- read_csv(paste0(out_path, "/Tier5_AKskt_NBSoutput.csv")) %>% 
  mutate(model_name = "Separate rema")
all_output <- read_csv(paste0(out_path, "/Tier5_AKskt_alloutput.csv"))%>% 
  mutate(model_name = "Combined rema")

graph_dat <- EBS_output %>% bind_rows(NBS_output) %>% bind_rows(all_output)

EBS_plot <- ggplot(data = graph_dat,
                      aes(x = year, y = pred,
                          col = model_name)) +
  ...
  geom_point(aes(x = year, y = obs), col = "black", show.legend = F) +
  geom_errorbar(aes(x = year, ymin = obs_lci, ymax = obs_uci), col = "black", show.legend = F) +
...
ggsave(path = out_path, "Tier5_AKskate_rema.png",
       plot=EBS_plot,
