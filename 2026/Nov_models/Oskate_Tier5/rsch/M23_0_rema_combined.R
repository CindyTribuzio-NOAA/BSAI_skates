# Tier 5 Models, testing combined rema
...
out_path <- here::here(AYR, 'Nov_models', 'Oskate_Tier5', 'rsch'); dir.create(out_path)
...
# prep data for rema  ----
Osktall_input <- prepare_rema_input(model_name = 'M23_Oskt_combined',
                            biomass_dat = all_dat,
                            end_year = pYEAR,
...
# fit rema models ----
m23_Osktall <- fit_rema(Osktall_input)
m23_Osktall_out <- tidy_rema(m23_Osktall)

# plot and clean up output ----
m23_Osktall_plots <- plot_rema(tidy_rema = m23_Osktall_out)


m23_Osktall_tot <- tidy_rema(m23_Osktall)$biomass_by_strata %>% 
  select(strata, model_name, strata, year, variable, pred, pred_lci, pred_uci, obs, obs_cv, obs_lci, obs_uci) 

write_csv(m23_Osktall_tot, here::here(AYR, 'Nov_models', 'Oskate_Tier5', 'rsch', 'Tier5_Osktall_output.csv'))

# make nice summary graph ----
Osktshared_output <- read_csv(paste0(out_path, "/Tier5_Osktall_output.csv")) %>% 
  mutate(model_name = "Combined rema")
Osktseparate_output <- read_csv(here::here(AYR, 'Nov_models', 'Oskate_Tier5', 'mgmt', 'Tier5_m23_output.csv')) %>% 
  mutate(model_name = "Separate rema")
Oskt_all_figdat <- Osktseparate_output %>% 
  bind_rows(Osktshared_output)


plot_sharedPE <- ggplot(data = Oskt_all_figdat,
       aes(x = year, y = pred,
           col = model_name)) +
  ...
  facet_wrap(~strata, ncol = 1, scales = "free") +
